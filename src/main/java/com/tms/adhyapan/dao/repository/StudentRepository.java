...
public interface StudentRepository extends JpaRepository<Student, Long> {
	...
	@Query(value = "select s from Student s, Clazz c, StudentClass sc where s.id = sc.studentId and sc.classId = c.id and c.id=?1 and s.isActive='Y' and c.isActive = 'Y' and sc.isActive = 'Y'")
